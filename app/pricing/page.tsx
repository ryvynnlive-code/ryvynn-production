...
const TIERS = [
  ...
  {
    id: 'therapist',
    nameKey: 'tierTherapist',
    icon: '💎',
    regular: 69.36,
    priceId: 'price_1TCvSnFXY1nWj7h7zOhi50a7',
    badgeKey: 'pricingBadgePro' as string | undefined,
    border: 'border-ryvynn-cyan',
    glow: 'shadow-[0_0_40px_rgba(0,217,255,0.3)]',
    ctaKey: 'pricingCtaUpgradePro',
    features: [
      '693 Soul Tokens/mo',
      'Client dashboards',
  ...
  },
  {
    ...
    icon: '🏢',
    regular: 96.36,
    priceId: 'price_1TCvSyFXY1nWj7h7aBMnrWOv',
    border: 'border-ryvynn-purple',
    glow: 'shadow-[0_0_40px_rgba(139,92,246,0.3)]',
    ctaKey: 'pricingCtaScaleTeam',
    features: [
  ...
  {
    id: 'lifetime',
    nameKey: 'tierLifetime',
    icon: '♾️',
    regular: 369.36,
    oneTime: true,
    priceId: 'price_1T83YxFXY1nWj7h7KLhYLVU3',
    badgeKey: 'pricingBadgeBestValue',
    border: 'border-ryvynn-purple',
    glow: 'shadow-[0_0_60px_rgba(139,92,246,0.5)]',
    special: true,
    ctaKey: 'pricingCtaBurnEternal',
    features: [
...
// Badge label fallback (therapist has no matching key, use raw string)
...
export default function PricingPage() {
  ...
  const getTierName = (tier: typeof TIERS[0]) => t(tier.nameKey as Parameters<typeof t>[0]);
  const getTierCta = (tier: typeof TIERS[0]) => t(tier.ctaKey as Parameters<typeof t>[0]);
  const getTierBadge = (tier: typeof TIERS[0]) => {
    if (!tier.badgeKey) return null;
    if (tier.id === 'therapist') return TIER_BADGE_LABELS[tier.id];
    return t(tier.badgeKey as Parameters<typeof t>[0]);
  };
            ...
            return (
              ...
              >
                ...
                <div className="relative">
                  {/* Icon */}
                  <div className={`mb-6 ${tier.special ? 'text-7xl' : 'text-5xl'} ${tierBadge ? 'mt-6' : ''}`}>
                    {tier.icon}
                  </div>
                  ...
                  >
                    {loading === tier.id ? `⚡ ${t('pricingProcessing')}` : `🔥 ${tierCta.toUpperCase()}`}
                  </button>
                </div>
              </div>
            );
